...
Rebuilds the trigger and hardens the no-impact claim, after a live incident in which the skill never loaded during a bug-fix session and a false "no documentation impact" line shipped while the affected spec page was cited by name in the very files the fix touched. The description now names the moments that need the skill instead of describing a domain: before writing that a change has no documentation, spec, or product impact; when product behavior changes or a system boundary is accepted; before opening or updating a pull request, closing a task, or ending the session. It states outright that the claim fails most often on bug fixes, refactors, and instrumentation nobody framed as documentation work. Changes confined to agent instructions, internal notes, or build tooling are named as out of scope.

A new evidence principle, "Prove a negative before writing one", makes the no-impact claim writable only with a receipt beside it: search terms harvested from the changed lines and each touched file's header comment first, the documentation search run with those names, and either the document the change cites or an explicit "the change cites no document", the one part a reviewer can check against the diff in seconds. Behavior that changed while no page owns it records as documentation debt instead of "none"; a change that touches nothing documentation covers files nothing. Pull request and delivery sync step 1 carries the same rule on the "none" line, where the receipt serves as the reason. Existing docs, marks, and workflows need no migration.

### 1.3.0 — 2026-08-22

Hardens Release sync into an explicit safety net: the pass now must end with updated docs or named documentation debt, never a silent skip, and each release keeps one durable record that the next pass reads. Two steps are new. The pass bounds the release window on the previous record's exact revision — never a moving branch head or a bare date, which silently drops same-day changes — over-collecting toward the older side when the boundary is imprecise and saying so. It then consumes the documentation-impact mark each PR now records (Pull request and delivery sync step 1 adds the recording rule, including the "none — technical-only" form): a named maintained doc joins the update list with its evidence, "none" is still checked against the change itself and counts as missing when the check fails, a missing mark is inferred or filed as debt, and a mark naming an unmaintained doc is not an edit order.

Canon handling is now unconditional: every pass resolves and opens each locked canon document (the Product Map or equivalent entry point, Product Facts) and assesses the release against its actual text; a canon document that cannot be found is a failed step to report, never a silent pass. Edits to canon remain draft-first — exact before and after in front of the accountable user, with an approved-but-unapplied edit preserved as a tracking task. The pass closes by recording the boundary covered, this release's exact revision, each doc updated with its evidence, each canon outcome, and the documentation-debt link. Existing docs, marks, and workflows need no migration; the new steps apply from the next release pass.
...
Rigour now has a dial that turns both ways. Planning records a stakes Decision — the decision the stream feeds, its blast radius, whether a wrong outcome can be undone — and every review floor reads it: a reversible, internal outcome holds the floor at one fresh lens and can size Verify slots down, and the supervisor honors a plan that subtracted on that record instead of overruling it. Review rounds get a budget: two finding-bearing rounds per task, reviewers judge convergence from round two, and a third round requires a written choice — accept the remaining class as a stated limit, re-plan, or ask the user.

Checks now point outward too. Every stream records a falsifiable kill question — what would make the whole output worthless, and what observation would show it — confirmed at the hand-off beside the outcome in countable units, re-answered by every resuming supervisor, and re-asked in writing on a fixed cadence inside the loop, together with a cost line that drops any check that outgrew the decision it protects. A task building an experiment, reproduction, fixture environment, or simulation carries a parity manifest built outward-in from the real system, closing a parity defect requires the full enumeration rather than the found instance, and a materially expensive run must pass a cheap rehearsal of its core premise before the spend. Reviews of any measurement, comparison, score, or verdict add a mandatory premise lens pointed upstream at the data consumed, allowed to rule the input unfit. Reports carry the definition of success's number and its current value, and quote observed behaviour verbatim, not only scores.

Premises stay re-checkable. A user's mid-delivery mandate change is now an explicit replanning trigger with two sweeps before the next dispatch: every open task spec re-read against the new mandate, and retired Decisions' artifacts swept for residue still running. A diagnosis inherited at handover is unverified until its own proof is re-run on current artifacts. The scoping brief adds a Readings part — how the planner read everything the user named ambiguously, with the rejected reading beside it — and the delta path may skip re-confirming scope but never the Readings. Decisions that fix an interpretation record the reading they rejected; Decisions constraining a rig state their observable test; oversized records become a page with the Decision as pointer. Disclosed effects touching shared resources carry their magnitude. The definition of success must contain at least one number. Escalation gets an ownership test so reversible technical calls stop reaching the user, every check ships with a defeat proof designed before it, and a challenged finding gets its primary source re-read before any defense.

Four independent transcript audits of the five supervisor sessions then showed the post-mortem's accounts of their own conduct were unreliable — at every leg the fatal fact was seen, written down, and neutralized rather than acted on — so the release adds disposition rules, not only detection rules. An observation that answers the stream's kill question stops the stream, whoever wrote it and wherever it sits: dispatching onto it, deferring it to a later task, recording it as a stated limit, and declaring an uneliminable difference to be the design are named as the four shapes of the same failure, and the finding-disposition machinery tests every finding against the kill question before any deferral. A distress signal from shared infrastructure pauses every task loading that resource, and takes precedence over the reduced-retry rules. A failed rehearsal stops the dispatch — reporting it is not clearing it. A go typed before the hand-off was shown is not a go: the go Decision must postdate and name the hand-off message it answers, resume checks the timestamps, and an escalation is asked once — a decision line unchanged across three task closes or four hours is a stall the supervisor owns, with checkpoints, person-waits and undisclosed-consent waits explicitly out of its reach. The verified-facts rule extends to every record and report a session writes: quotes copied from sources read this session, numbers re-derived at the moment of writing, moving numbers replaced by pointers.

Three operator findings from the same run land alongside. Planning groundwork now maps the full path to the finished state, accesses included: every credential, secret, account, and remote surface the delivery itself will need, with anything only a person can provide wired as a human-only task before its dependents — asked at the go, never discovered mid-run with the stream stalled behind it — and the gate map's person-wait class widened to hold such steps. Every delivery report opens with a position-and-outcome line — task N of M executable tasks, the current phase, and the success metric's current value — so the longest stage the user watches is never opaque. And local versus remote is stated never to be what decides autonomy: a disclosed remote effect is as authorized by the go as a local edit under the same house-rules floor, and what stops a remote step is exactly what stops any step.

A joint consolidation with the Codex distribution's audit of its own five runs adds the last layer. Authority is monotonic inside a rebalance — a valid go persists through internal corrections, and new consent is owed only for a new protected effect, an unnamed target, or a changed user gate's substance, never for dependency or gate re-sequencing. A review round's record or an authorization is never overwritten: superseding records point at predecessors, which keep their findings and timestamps, with the go record's two guarded who-is-acting fields exempt. A protected effect is re-checked at the moment of landing against the current spec, the go record, and the contract. Closing the stream is an agreement — task states and tracker logic matching reality, the supervisor line cleared, any failed closing call disclosed. Resuming supervisors reconcile standing authority, pending effects, live workers, and person-waits before the first dispatch. Every Verify slot names its proof surface, and a source read or passing suite never satisfies a rendered-page or live-system Done when. Trackers and auto-generated views are never sources or authorizations. Remote surfaces are proven reachable before the go with a read or dry call, never the effect itself. A run whose outcome number stops moving for two closes must explain itself at the next kill-question check. And a downsized review floor is visible: the hand-off says it in plain words, the remaining reviewer attacks the stakes record itself, and a call-site proof omitted on that record says so in the Verify slot — a silent absence is not a downsizing.
...
The early delivery-contract questionnaire is gone. The planner still reads the target's house rules at the same moment and still records the gate map, reviewer routing, and stacking as stream Decisions, but settles them alone; between the brief confirmation and the finished plan the user is interrupted only by decisions no one else can own. The gate map now separates person-waits — points the target's own rules route to a named person, honored in every mode — from protected effects: schema and migration work, permission and security surfaces, publishing, deploying, deleting data, any spend, and anything else that cannot be undone.

The hand-off's plan list discloses every protected effect in bold consequence words and closes with one question that is also the go: full autonomy, checkpoints, or step-by-step. These delivery modes replace the two autonomy dials. The go Decision copies the plan list's bold lines verbatim — they are the disclosed effects and the marked points, and the only test a later session has of what the go covers. Under full autonomy, delivery runs uninterrupted through everything the plan disclosed; only an undisclosed event — a protected effect missing from that list, a changed outcome, or an authority need the plan never named — a person-wait, or a failure that survives its retries reaches the user, each stated as its consequence. Checkpoints adds a pause at each bold line; step-by-step pauses after every task. No mode lowers a check, a review floor, or the house rules.

Every user-facing message now opens with a fixed stage banner — Octoplan · Step 1 of 3 — Brief, then Plan, then Delivery — shared across Octoplan implementations. The scoping brief scales to the request but is never skipped. The plan review floor is one fresh reviewer, always; a plan of 8+ tasks, or one touching migrations, permissions, privacy, money, publishing or deploying, or destructive operations, requires two or more single-lens reviewers. The delivery state vocabulary extends to released and accepted, and continuation blocks now carry solo or parallel-safe in their settings line.

Migration, which makes this a major release: streams planned under 0.x record two autonomy dials, and their old authority is never silently widened. A resuming supervisor re-authorizes the stream: it shows the full current plan with its consequence lines, runs the review the plan has not had under this contract, asks the one mode question, and records a fresh go with the disclosed list and the reviewed graph. A user request to re-plan, or a brief reality has left behind, now explicitly routes a resume back to planning.
