...
The control plane now fingerprints exact stream and task topology, review triggers, routes, effects, checkpoints, rules, and source versions. It enforces the shared plan and delivery review floors, trigger-specific independent lenses, version-bound continuation receipts, immediate evidence persistence, shared or domain-equivalent work-state vocabulary, and integrated closure. Plan-review receipts are drafted per judgment and become durable only after the full review floor reaches PASS. Recovery loads before a duplicable or hard-to-undo external effect, records its stable action key before execution, and activates after two no-progress cycles. Closing recaps name residual risks and findings deferred with authority and rationale. Deterministic fixtures fail closed on drift, stale authority, missing review evidence, duplicate reviewer identity, routing ambiguity, missing effect keys, non-idempotent retries, unsafe takeover, skipped interruption obligations, and unsupported closure. `CONFORMANCE.md` maps each guarantee to its trigger point.
