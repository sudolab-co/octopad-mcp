...
5. **Report.** Update the delivery report where one exists, then post to the user at most one plain line per event since the last report — even under full autonomy; silence is not a progress report. Name the state, never a bare "done": built, reviewed, merged, applied, verified, released, accepted — stopping where the stream's finish line is. No inner monologue: what was verified, not how you reasoned about it. One thing never compresses: a call you made that the user could reasonably disagree with — a finding you dismissed, an item you decided was yours or theirs, a deviation from the plan — gets its own line in chat when you make it. Review findings live on the task — the top one may get one line in chat, never a narration. Every blocker you name says what it stops and what keeps moving. A standing blocker is named once, then lives on the report; repeat it in chat only when its state changes or it comes to block everything. When you repeat a task's precondition, carry its reason. When you hand a person a task, say it already holds everything they need and name where it sits.
...
Spawn ONE sub-supervisor subagent — fresh, never a fork, because a fork inherits this session's full conversation, spent budget included, which is the problem being escaped — at the supervisor route recorded in the go Decision, by lane where lanes exist, never substituting the model. Open its prompt with the two lines of the supervisor handoff block (`Octoplan Autopilot <work stream>` plus the Octopad address line), so it invokes this skill and briefs itself from Octopad exactly as a pasted block would: the recorded go, the contract Decisions, the open tasks and their comments. Under those two lines add only what is true of the environment and held nowhere in Octopad — the branch the chain stacks on, and nothing more: the task in flight was closed out or parked with its state written on it before delegating, like any handover. Every rule in this file binds the sub-supervisor as if it were the supervisor, with one exception: it never delegates — when its own context thins it reports that like any other state, and the relay retires it and spawns its replacement the same way.
