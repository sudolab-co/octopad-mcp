...
On a multi-stream effort, also read the effort's Blueprint page before the first task and after any interruption — it carries the effort's shape, the shared-landing order, and the cross-stream rules from the skill's Multi-stream efforts section. Your stream is your whole territory: rebalance only it, and stop for the user on anything touching a seam.
