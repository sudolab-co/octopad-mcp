...
11. **Hand off.** Rename the work stream so its name ends with ` (octoplanned)` — skip if it already does — so anyone scanning the workspace sees which streams have been through a full Octoplan pass. The suffix is a marker for humans reading the workspace, not part of the stream's identity: continuation prompts always use the plain name. Then end with the hand-off message, in this fixed order every time, so the user recognizes the moment across streams. Do NOT restate chain state or the team's standing rules in it: state lives in Octopad, rules live in the team's own instruction files. If you're tempted to add a fact to the handoff, it belongs in an Octopad task, Decision, or knowledge item — put it there.
    ...
    5. **The prompt block for the recommended path**, with its settings line (model · effort · solo or parallel-safe) under it, per Continuation. Fresh supervisor recommended → the supervisor handoff block, told plainly to paste it only after the go is recorded (the receiving session checks for the recorded go and only re-runs planning without it). This session supervising → no block; say the go starts delivery right here. User chose the manual task-by-task path → the first ready task's block instead.
    On the user's explicit go, first record the go itself as a Decision on the stream — and record the supervisor route (model · effort, solo or parallel-safe) in that same Decision, because the delivery offer exists only in this chat and a replacement supervisor must find its route in Octopad. That record is what lets a later session know it may resume, and it is what makes handing off safe: the fresh supervisor reads the go as permission already given and never asks for it again. Then either load `references/supervision.md` and follow it, or end with the supervisor handoff block (see Continuation). Without a go, nothing else happens here: the plan sits in Octopad and the continuation prompt is how it moves.
...
**Every block handed to the user in chat carries its launch settings.** Whenever a session gives the user a block to paste — the planner's hand-off, a supervisor handing over, an executor emitting the next task's block — one plain line goes directly under the fence (outside it): the model and reasoning effort the new session should run at, and solo or parallel-safe. For a task block those come from that task's saved Exec line, which the emitting session reads in Octopad when it checks the task is ready; for a supervisor block, from the supervisor route recorded in the go Decision (step 11). The user launching the session is the one who sets model and effort, so a block without this line hands them a decision the plan already made. The Next patterns below carry this instruction in their own words, because the session that emits a block from a Next line has never read this file.
