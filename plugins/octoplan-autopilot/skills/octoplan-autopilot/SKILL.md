...
Before planning anything, look at the stream you were invoked on. If ALL THREE hold — its name already ends with ` (octoplanned)`, it carries the delivery contract's Decisions, and one of those Decisions records a delivery go the user has not withdrawn — then this is a resume, not a new plan. Do not re-plan, do not re-run the brief, do not re-ask for the dials, and do not re-ask for the delivery go: the recorded go is the user's permission, already given. Load `references/supervision.md`, re-read the contract Decisions, the open tasks and each one's latest comment, and rejoin the delivery loop where it stopped.
