...
11. **Hand off.** Rename the work stream so its name ends with ` (octoplanned)` — skip if it already does — so anyone scanning the workspace sees which streams have been through a full Octoplan pass. The suffix is a marker for humans reading the workspace, not part of the stream's identity: continuation prompts always use the plain name. Then end with a short wrap-up (what the plan contains — task count, decisions locked, open questions) and the continuation prompt for the first ready task, in the exact fenced format defined in Continuation. Do NOT restate chain state or the team's standing rules in the handoff: state lives in Octopad, rules live in the team's own instruction files. If you're tempted to add a fact to the handoff, it belongs in an Octopad task, Decision, or knowledge item — put it there.
    Two things ride with that wrap-up. First, **the gate-map delta**: the contract's forecast (step 3) was made before the tasks existed, so name in a few lines every human gate the specced plan actually carries — which ones the forecast missed, which no longer apply — and let the user correct it. Update the contract Decisions with whatever they correct. Second, **the delivery offer**: ask whether this session should now become the supervisor and deliver the plan under the confirmed contract. On the user's explicit go, load `references/supervision.md` and follow it. Without a go, nothing else happens here: the plan sits in Octopad and the continuation prompt is how it moves.
...
Under supervised delivery these blocks are the **recovery path, not the normal one**: the supervisor sequences the work itself and its workers emit no continuation block (see `references/supervision.md`). They still get written on every task, because they are what saves the stream if the supervising session dies — the user pastes the block for the next open task into a fresh session, which reads the contract Decisions and the task comments from Octopad and carries on.
