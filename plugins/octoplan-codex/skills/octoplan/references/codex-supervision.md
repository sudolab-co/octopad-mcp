...
After valid launch authority and before any claim, verify this session's actual model, effort, target, and environment equal the saved inline route and inline supervisor target. Authority is either a later explicit yes on the reviewed plan or an advance authorization plus its guarded launch-binding record. For advance authority, reread the source reply and binding record and require the confirmed-brief digest, exact final hash, review PASS, saved-state equality PASS, and no-material-delta assertion to match current durable state; any mismatch stops. Then guardedly append the first run record: unique run ID, exact approved hash, authority mode, approval source thread and time, launch-binding record when applicable, state `active`, current mode, route, target, and environment, fresh inline owner token, supervisor epoch 1, zero dedicated replacements used, and no candidate. The approved hash must still equal the manifest and live plan.
