...
After create, list/read every candidate. `clientThreadId` is setup only; wait boundedly while nonterminal. Missing, unreadable, malformed, timed-out, or conflicting identity is `unavailable`, never null.

Compare immutable creation and actual project identity from native metadata/registry; prompt text is never proof. Nonterminal unavailable/verified-null stays `pending` without activation, handoff, or create. Ready/terminal verified null alone permits repair: persist intent, invoke once, persist `operationId`, await success, and adopt only `destinationThreadId`; setup/source IDs never qualify. Error, timeout, changed/missing destination, unavailable/null-after-repair, projectless, or wrong project pauses with the six-field handoff and forbids repeats. Gate identity separately on current epoch. Zero matches stay pending; one exact destination becomes ready; multiple/conflicting matches pause.
