...
Before a post-plan native actor, persist exactly one guarded authority record for the current plan:

```json
{"schema":"octoplan-native-grant-v1","record_id":"<non-empty unique record ID>","plan_hash":"<lowercase SHA-256>","project_identity":{"kind":"<project|projectless>","project_id":"<ID or null>","directory_name":"<name or null>"},"environments":["<local|worktree or null>"],"roles":["<supervisor|planner|executor|lead-reviewer|specialist-reviewer|recovery|follow-up>"],"source":{"record_id":"<non-empty exact message record ID>","message_digest":"<lowercase SHA-256>"}}
```

Keys are exact. Project identity needs non-empty project ID, null directory, and a non-empty unique scalar-sorted subset of `local|worktree`; projectless uses null project, non-empty directory, and `[null]`. Roles are non-empty, unique, scalar-sorted, and listed above. Source is the runway's exact user-message record/digest, never Delivery mode, consent, or another grant. The record stays outside the fingerprint but binds its final hash. A replan never edits, copies, widens, or silently rebinds it; a new hash needs a new guarded record and covered source.
...
The first prompt line is literal `OCTOPLAN_CREATION`, one ASCII space, then JSON; the role packet is immutable. Packet, capability, source, route, capacity, or target mismatch pauses. The durable record stores identity/state/wake; its key excludes token and creator epoch. Supervisor/follow-up use null attempt; task roles non-null; reviewers require artifact revision. Before `intent`, match the inline target. After creation, classify authoritative native identity only as verified project, verified projectless, verified null, or unavailable; prompt text proves nothing and `clientThreadId` is only setup. Nonterminal unavailable/null stays `pending` without activation, handoff, or create. Only ready/terminal verified null permits one persisted repair and native handoff. Persist `operationId`, await terminal success, and adopt only `destinationThreadId`; source/setup IDs never match. Error, timeout, changed/missing destination, unavailable/null-after-repair, projectless, or cross-project evidence pauses before activation and forbids repeats. At `intent`, immutable `creator_owner_epoch` equals current epoch; takeover increments only current epoch. Activation/ledger writes require equality. Match immutable identity, gate current epoch separately, save intent before one create, and reconcile every response/crash.
