...
Use stable `E01` refs for delivery and `H01` refs only for protected gates represented by separate human tasks. Human review and PR merge are not separate Octopad tasks: embed both as first-class gate occurrences in the owning delivery task. Every gate records `gate_key`, `kind`, `location`, `delivery_task_ref`, `owner`, `target_effect`, `evidence_ref`, `state`, and `resume_predicate`. Embedded review/merge use `location=embedded`; other protected gates may use `location=human-task` with a distinct Hxx. Agent review is a native session, never an Octopad task.
...
The delivery task containing embedded human review/merge stays active through both gates. Its Done when includes review, merge, and closure evidence. Follow active `AGENTS.md` and repository workflow. Do not create login, probe, read, tracker, status-relay, human-review, merge, or tool-call tasks. Combine steps sharing owner, artifact, route, verifier, and gate unless independently acceptable.
