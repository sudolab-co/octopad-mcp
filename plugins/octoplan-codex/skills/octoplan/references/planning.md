...
Plan-only work stops without a Goal. For authorized Delivery, choose one supervisor and record it, and its Goal if used, in a guarded stream Decision. Continue here only with enough context to verify the run; otherwise use this handoff:
...
The receiving task applies handoff acceptance from [recovery.md](recovery.md), confirms its predecessor stopped, then applies the optional Goal rule in [codex-runtime.md](codex-runtime.md). Never alter an unrelated Goal or ask for another go without an authority delta.
