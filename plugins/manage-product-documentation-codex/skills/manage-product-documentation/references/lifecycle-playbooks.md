...
1. Determine documentation impact: none, existing-system change, new system, or technical-only. Record it on the PR itself in the repository's normal way — a documentation-impact line naming the affected doc, or an explicit "none" with a reason (technical-only records as "none — technical-only" plus the reason) — so release sync can consume it later. Reach `none` last, and prove it. Before writing it, read the changed hunks and each touched file's header comment for the specs, pages, or sections they already name; search the documentation for those names and for the product terms of the change; open what returns rather than judging it by title. Put the receipt on the `none` line itself, where it serves as the reason: the terms searched, the pages returned or `no candidate`, and either the document the change cites or `the change cites no document`. When behavior changed and no page owns it, that is documentation debt rather than `none`: say so on the same line and create or reuse the debt Task from [artifact-shapes.md](artifact-shapes.md). When the change touches nothing documentation covers, `none` stands with its receipt and files nothing.
