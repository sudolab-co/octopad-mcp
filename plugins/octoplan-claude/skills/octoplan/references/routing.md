...
| Genuinely open design or long-horizon problem where the approach itself is uncertain | **Fable 5.1 · xhigh**, only after confirming availability and that its mandatory 30-day data retention is acceptable; otherwise **Opus 5 · xhigh** |
...
Every Fable 5.1 recommendation, at any effort, requires confirmed availability and acceptance of its mandatory 30-day data retention. If either condition fails, use Opus 5 at the best compatible effort for the task.

## Effort vocabulary

| Setting | Octoplan policy |
|---|---|
| `low` | Native effort, but do not recommend it for Sonnet 5, Opus 5, or Fable 5.1 work. |
...
| `high` | Default substantive route for Opus 5. It may be used with Fable 5.1 when the user already chose Fable for a bounded capability-sensitive task. Never use it for Sonnet 5. |
...
A required Review line names its lens count in its why: one fresh reviewer by default; two or three single-lens reviewers where a mistake cannot be taken back (schema, permissions, privacy, money, publishing). **Proportionality reads the stream's stakes Decision and moves in both directions:** where the outcome the stream feeds is recorded as reversible and internal, the floor is ONE fresh lens — pressure to add rounds routes through the supervisor's review-round budget (`references/supervision.md`) instead of accumulating; where a mistake cannot be taken back, lenses are added to risk. The one thing no stakes level removes is the single fresh review a material change gets, and the delivery mode never lowers any of it. Where the task consumes or produces another task's artifact, one lens is the wiring — does the composed path run in production, not just in a fixture. A second lens that would re-read the same work with the same question is a duplicate, not a check. The supervisor — or the executor on the manual path — may add a lens, and may drop one only for a re-review narrower than the round before, naming which and why on the task.

**The premise lens — mandatory whenever the reviewed output is a measurement, a comparison, a score, or a verdict, and always pointed upstream.** It does not attack the deliverable: it attacks what the deliverable consumed. Its mandate, in these words or equivalent: *ignore the deliverable; examine the data and the instrument it was computed from, and state whether that data can support any conclusion in either direction.* It is explicitly allowed to return "the input is unfit and no finding about this deliverable matters" — a verdict every claim of which traces perfectly to evidence is still void when the instrument measured the wrong thing, and a lens scoped to the artifact in front of it can never see that. This lens rides alongside whatever the Review line names; it is not one of the counted lenses above, and the stakes record never removes it — a reversible stream's measurement is still a measurement.
