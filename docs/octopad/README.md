...
The release is published through a reviewed branch and PR in `sudolab-co/octopad-mcp`, tags `octopad-claude-vX.Y.Z` and `octopad-codex-vX.Y.Z`, releases `Octopad X.Y.Z (Claude Code)` and `Octopad X.Y.Z (Codex)` for each version, with fresh checks of the exact revision. Version 1.0.0 keeps its `octopad-v1.0.0` tag. Source publication, client installation and kernel activation are separate outcomes. See the release page for publication state and the validation record for obtained and missing witnesses.
