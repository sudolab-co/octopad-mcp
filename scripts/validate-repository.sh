...
grep -Fq '| [`octoplan-codex`](plugins/octoplan-codex/skills/octoplan/SKILL.md) | Codex | 17.1.0 | Builds a lean governed plan, challenges it once, then supervises authorized delivery from Octopad. |' "$root/README.md" || fail 'README Codex version or behavior is stale'
...
grep -q '"version": "17\.1\.0"' "$root/plugins/octoplan-codex/.codex-plugin/plugin.json" || fail 'Codex plugin is not 17.1.0'
grep -q '^Version: 17\.1\.0$' "$root/plugins/octoplan-codex/skills/octoplan/SKILL.md" || fail 'Codex skill is not 17.1.0'
...
grep -q '^### 17\.1\.0 — 2026-08-14$' "$root/CHANGELOG.md" || fail 'Codex 17.1.0 entry is missing'
