...
grep -Fq '| [`octoplan-codex`](plugins/octoplan-codex/skills/octoplan/SKILL.md) | Codex | 17.0.0 | Builds a lean governed plan, challenges it once, then supervises authorized delivery from Octopad. |' "$root/README.md" || fail 'README Codex version or behavior is stale'
...
if (claudeManifest.name !== 'manage-product-documentation' || claudeManifest.version !== '1.1.0' || claudeManifest.license !== 'MIT') process.exit(1);
...
grep -q '"version": "17\.0\.0"' "$root/plugins/octoplan-codex/.codex-plugin/plugin.json" || fail 'Codex plugin is not 17.0.0'
grep -q '^Version: 17\.0\.0$' "$root/plugins/octoplan-codex/skills/octoplan/SKILL.md" || fail 'Codex skill is not 17.0.0'
