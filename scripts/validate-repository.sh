...
grep -Fq '| [`octoplan-codex`](plugins/octoplan-codex/skills/octoplan/SKILL.md) | Codex | 18.1.0 | Confirms a Brief, reviews the Plan, then supervises authorized Delivery at the chosen interruption level. |' "$root/README.md" || fail 'README Codex version or behavior is stale'
...
grep -q '"version": "18\.1\.0"' "$root/plugins/octoplan-codex/.codex-plugin/plugin.json" || fail 'Codex plugin is not 18.1.0'
grep -q '^Version: 18\.1\.0$' "$root/plugins/octoplan-codex/skills/octoplan/SKILL.md" || fail 'Codex skill is not 18.1.0'
