...
grep -Fq '"title": "<stream title>"' "$planning" || fail 'fingerprint omits the saved stream title'
grep -Fq 'The concrete `impact` value is a JSON number' "$planning" || fail 'fingerprint leaves impact JSON type ambiguous'
grep -Fq 'execution environment block with observed host, path, and Git evidence removed' "$planning" || fail 'fingerprint can include non-binding environment evidence'
...
grep -Fq 'For a multi-stream dedicated supervisor, use the plain name of the ledger-owning stream.' "$supervision" || fail 'multi-stream supervisor title is ambiguous'
grep -Fq 'Every executor uses the plain name of the stream that owns its task.' "$supervision" || fail 'multi-stream executor title can use the wrong stream'
grep -Fq 'full creation key and token; do not embed a proposed epoch' "$supervision" || fail 'dedicated supervisor prompt lacks a full recoverable identity'
