...
**Two-tier workflow.** A planning session (this skill) produces the plan. Execution happens in ordinary work sessions: each one briefs itself from Octopad, does its one task, and follows the hand-off instruction the planner wrote into that task. Executors never load this skill — **everything an executor must do lives in the task descriptions the planner writes**, which is why the templates below spell the hand-off instructions out verbatim. The planner plans, never implements.
...
8. **Hand off.** Rename the work stream so its name ends with ` (octoplanned)` — skip if it already does — so anyone scanning the workspace sees which streams have been through a full Octoplan pass. Rename BEFORE minting any continuation prompt, so every prompt carries the stream's final name (Octopad's name matching is fuzzy, so an older prompt still resolves — but don't rely on it). Then end with a short wrap-up (what the plan contains — task count, decisions locked, open questions) and the continuation prompt for the first ready task, in the exact fenced format defined in Continuation. Do NOT restate chain state or the team's standing rules in the handoff: state lives in Octopad, rules live in the team's own instruction files. If you're tempted to add a fact to the handoff, it belongs in an Octopad task, Decision, or knowledge item — put it there.
