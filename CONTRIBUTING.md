...
A distribution's folder name, its plugin `name`, and its release tag prefix are the same string. A plugin may aggregate related skills: `octopad` is the Codex-first bundle, with ten satellite skills and one technical bootstrap. The plugin identity does not rename its skills. A skill that ships to more than one AI runtime carries that runtime in the name; a skill that ships to one runtime does not. Release titles read `<Display Name> <version>`, nothing else.
