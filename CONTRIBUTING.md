...
2. **The plugin's `version`**, in `plugins/<plugin>/.claude-plugin/plugin.json`. Same number.
...
Tags carry the skill name because each skill in this repo is versioned on its own.
