...
Octoplan for Claude Code now ships as one distribution, `octoplan-claude`, at version `2.0.0`. Two earlier Claude identities are retired: the planning-only skill that carried the `octoplan-claude` name, and the experimental `octoplan-autopilot`. Remove whichever is installed before adding the current one.
